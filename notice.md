...
Естественно, однобуквенных и коротких названий следует избегать (кроме, например, переменной цикла `i`, когда нет вложенности циклов `for`).
...
Названия полей класса стоит `m_начинатьСБуквыMЧтоЗначитMember`.

Названия глобальных переменных стоит `g_начинатьСБуквыGЧтоЗначитGlobal`.

Названия переменных, содержащие цифры, лучше не использовать (исключение: параметры оператора сравнения можно назвать, например, так: `item1` и `item2`).
